...
module.exports.getValidator = function (request, response, next) {
    request.checkQuery('geo.lat', 'No valid latitude provided').optional().isNumeric();
    request.checkQuery('geo.long', 'No valid longitude provided').optional().isNumeric();
    request.checkQuery('geo.dist', 'No valid distance provided').optional().isNumeric();
...
module.exports.postValidator = function (request, response, next) {
    ...
    request.checkBody('geo.lat', 'No valid latitude provided').optional().isNumeric();
    request.checkBody('geo.long', 'No valid longitude provided').optional().isNumeric();
    request.checkBody('geo.dist', 'No valid distance provided').optional().isNumeric();
    commonFunctions.validate(request,response,next);
};
