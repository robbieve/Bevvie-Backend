...
module.exports.winstonCategoryLogger = function (categoryName, fullConfig) {
    let config = {
        transports: [
            new winston.transports.File({
                name: categoryName,
                label: categoryName,
                level: "debug",
                filename: "logs/" + categoryName + "-logger.log",
                handleExceptions: true,
                humanReadableUnhandledException: true,
                json: false,
                maxsize: 8000000, //8MB
    ...
    winston.loggers.add(categoryName, config);
    let logger = winston.loggers.get(categoryName);
    return logger;
};
