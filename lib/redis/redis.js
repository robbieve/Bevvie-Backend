...
redis.on('error', function (err) {
    logger.debug("REDIS: Error: " + JSON.stringify(err));
});
...
module.exports.getCachedResults = function _getCachedResults(query, modelName, request, callback) {
    if (!config.enabled ||
        ...
        logger.debug(`CACHE: Disabled ${JSON.stringify(request.headers)}`);
...
module.exports.setCachedResult = function _setCachedResult(query, modelName, json, expireTime,callback) {
    if (!config.enabled) {
    ...
    if (!expire){
        expire = config.expireTime ? config.expireTime : 60 * 60 * 24;
    }
